...
class SleepQualityFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        val keyToValutate = SleepQualityFragmentArgs.fromBundle(arguments!!).sleepNightKey
        val dataSource = SleepDatabase.getInstance(application).sleepDatabaseDao


        viewModelFactory = SleepQualityViewModelFactory(keyToValutate, dataSource)
        viewModel = ViewModelProviders.of(this, viewModelFactory).get(SleepQualityViewModel::class.java)

        binding.dataSleepQualityViewModel = viewModel
        binding.setLifecycleOwner(this)

        viewModel.endQuality.observe(this, Observer { flag ->
            if (flag == true) {
                this.findNavController().navigate(SleepQualityFragmentDirections.actionSleepQualityFragmentToSleepTrackerFragment())
                viewModel.endValutation()
            }
        })
